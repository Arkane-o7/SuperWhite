#!/usr/bin/env python3
"""Convert a square SDR logo into a Rec.2020 PQ HDR JPEG.

The conversion lifts near-white pixels by a chosen number of exposure stops,
PQ-encodes the result, and embeds a Rec.2020 PQ ICC profile. The source image
...
def source_gamut(image: Image.Image) -> tuple[np.ndarray, str]:
    ...
    if "2020" in description or "PQ" in description:
        raise ValueError(
            f"input already looks HDR-encoded ({description}); use the original SDR logo"
        )
...
def validate_image(image: Image.Image) -> None:
    if image.width != image.height:
        raise ValueError(
            f"input is {image.width}x{image.height}; provide a square PNG or JPEG"
        )
    if "A" in image.getbands():
        alpha = np.asarray(image.convert("RGBA"))[..., 3]
        if alpha.min() < 250:
            raise ValueError(
                "input has transparency; flatten the logo onto a solid background first"
            )
...
def convert(input_path: Path, output_path: Path, stops: float) -> dict[str, object]:
    ...
    with Image.open(input_path) as image:
        ...
        encoded = np.asarray(image.convert("RGB")).astype(np.float64) / 255.0
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Make near-white logo pixels render as HDR highlights."
    )
    parser.add_argument("input", type=Path, help="square SDR PNG or JPEG")
